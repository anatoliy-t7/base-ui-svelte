<script lang="ts">
	...
	let {
		...
		class: className,
		style,
		id = useId('context-menu-submenu'),
		children,
		...rest
	}: ContextMenuSubmenuRootProps = $props();

	const parentMenu = getContext<ContextMenuContext>(CONTEXT_MENU_CONTEXT);
	const menuId = useId('context-menu-submenu-root');
	...
	const popupId = useId('context-menu-submenu-popup');
