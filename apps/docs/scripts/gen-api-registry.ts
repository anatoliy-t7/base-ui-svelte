...
import {
	findBaseUiPart,
	loadBaseUiComponentApi,
	mergePartWithBaseUi,
	preloadBaseUiApis,
	type BaseUiComponentApi
} from './sync-base-ui-api.js';
...
function baseUiApiForPart(
	slug: string,
	part: PartDef,
	loadedApis: Map<string, BaseUiComponentApi | null>
): BaseUiComponentApi | null {
	if (part.heading.startsWith('RadioGroup.')) {
		return loadedApis.get('radio') ?? null;
	}
	if (part.heading.startsWith('ToggleGroup.')) {
		return loadedApis.get('toggle-group') ?? null;
	}
	return loadedApis.get(slug) ?? null;
}

async function main(): Promise<void> {
	const refresh = process.argv.includes('--refresh');
	const preloadSlugs = [...new Set([...docsRoutes, 'toggle-group'])].sort();
	await preloadBaseUiApis(preloadSlugs, refresh);

	const loadedApis = new Map<string, BaseUiComponentApi | null>();
	for (const slug of preloadSlugs) {
		loadedApis.set(slug, await loadBaseUiComponentApi(slug, refresh));
	}
	...
	for (const [slug, api] of Object.entries(registry)) {
		registry[slug] = {
			...api,
			parts: api.parts.map((part) => {
				const baseUiApi = baseUiApiForPart(slug, part, loadedApis);
				const baseUiPart = findBaseUiPart(baseUiApi, part);
				return mergePartWithBaseUi(part, baseUiPart);
			})
		};
	}
